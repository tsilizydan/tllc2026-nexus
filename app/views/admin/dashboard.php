...
<div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-8">
    <?php foreach ([['users','Utilisateurs','users','purple'],['tasks','Tâches','check-circle','cyan'],['projects','Projets','project-diagram','emerald'],['contacts','Contacts','address-book','amber'],['notes','Notes','sticky-note','red'],['messages','Messages','envelope','blue'],['subscribers','Abonnés','newspaper','teal']] as [$key,$label,$icon,$color]): ?>
...
<div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
    ...
    <a href="<?= url('/admin/messages') ?>" class="card p-4 flex items-center gap-3 hover:border-primary-500/30"><i class="fas fa-envelope text-blue-400"></i><span class="text-sm text-white">Messages</span><i class="fas fa-chevron-right text-xs text-slate-600 ml-auto"></i></a>
    <a href="<?= url('/admin/newsletter') ?>" class="card p-4 flex items-center gap-3 hover:border-primary-500/30"><i class="fas fa-newspaper text-teal-400"></i><span class="text-sm text-white">Newsletter</span><i class="fas fa-chevron-right text-xs text-slate-600 ml-auto"></i></a>
